...
# Load training pairs
train_file = "reports/train.en-si"  # Update with actual file path
sinhala_train_pairs = load_text_pairs(train_file)
...
vectorized_sinhala = target_vectorization(np.array(train_sinhala_texts))

# Save vectorized data to a file
def save_vectorized_data(file_path, english_vectors, sinhala_vectors):
    with open(file_path, "w", encoding="utf-8") as f:
        for eng, sin in zip(english_vectors.numpy(), sinhala_vectors.numpy()):
            f.write(json.dumps({"english": eng.tolist(), "sinhala": sin.tolist()}) + "\n")

# Save the vectorized output
vectorized_output_file = "reports/vectorized_output.json"
save_vectorized_data(vectorized_output_file, vectorized_english, vectorized_sinhala)
